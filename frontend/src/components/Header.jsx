import { Shield, Monitor, Smartphone, Info, LayoutDashboard } from 'lucide-react';

export default function Header({ activeTab, setActiveTab, hasActiveReport, onViewReport }) {
  return (
    <header className="flex-none px-4 sm:px-6 py-3 sm:py-4 bg-[#050507]/80 backdrop-blur-md border-b border-zinc-800/50 shadow-lg z-50">
      <div className="max-w-7xl mx-auto flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {/* TAB SWITCHER */}
        <div className="flex p-1 bg-zinc-950 border border-zinc-800/50 rounded-lg w-full sm:w-auto shadow-[0_4px_20px_rgba(0,0,0,0.5)] overflow-x-auto">
          <button
            onClick={() => setActiveTab('dashboard')}
            className={`flex-1 sm:flex-none px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded-md text-[10px] sm:text-xs font-bold uppercase tracking-wider transition-all duration-300 flex items-center justify-center gap-1.5 shrink-0 ${
              activeTab === 'dashboard' 
          ...
          >
            <LayoutDashboard className="w-3 sm:w-3.5 h-3 sm:h-3.5 shrink-0" />
            <span className="truncate">SOC Cases</span>
          </button>
          ...
          {hasActiveReport && (
            <button
              ...
              className={`flex-1 sm:flex-none px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded-md text-[10px] sm:text-xs font-bold uppercase tracking-wider transition-all duration-300 flex items-center justify-center gap-1.5 shrink-0 ${
                activeTab === 'report' 
                  ? 'bg-zinc-900 text-cyan-400 shadow-[0_0_15px_rgba(34,211,238,0.15)] border border-cyan-500/50' 
                  : 'text-cyan-500/80 hover:text-cyan-300 hover:bg-zinc-900/60'
              }`}
            >
